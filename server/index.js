...
import pg from 'pg';
import { requirePermission, hasPermission, ROLES, normalizeRole } from './rbac.js';
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id:a.id, email:a.email, name:a.name, role:normalizeRole(a.role) }, JWT_SECRET, { expiresIn:'12h' });
    res.json({ token, user:{ id:a.id, name:a.name, email:a.email, role:normalizeRole(a.role) } });
...
app.get('/api/dashboard', ...protect('dashboard:read'), async (_req,res)=>{ try { const [total,pending,completed,messages,recent]=await Promise.all([query('SELECT COUNT(*)::int AS n FROM bookings'),query("SELECT COUNT(*)::int AS n FROM bookings WHERE status IN ('new','pending')"),query("SELECT COUNT(*)::int AS n FROM bookings WHERE status='completed'"),query("SELECT COUNT(*)::int AS n FROM messages WHERE status='unread'"),query('SELECT b.*,s.name_en AS service_name FROM bookings b LEFT JOIN services s ON s.id=b.service_id ORDER BY b.created_at DESC LIMIT 8')]); res.json({stats:{total:total.rows[0].n,pending:pending.rows[0].n,completed:completed.rows[0].n,unreadMessages:messages.rows[0].n},recent:recent.rows}); }catch(e){res.status(500).json({error:e.message});} });

app.get('/api/bookings', ...protect('bookings:read'), async (req,res)=>{ try { const q=String(req.query.search||'').trim(), status=String(req.query.status||'').trim(); const p=[],w=[]; if(q){p.push(`%${q}%`);w.push(`(b.customer_name ILIKE $${p.length} OR b.pet_name ILIKE $${p.length} OR b.booking_code ILIKE $${p.length})`);} if(status){p.push(status);w.push(`b.status=$${p.length}`);} const sql=`SELECT b.*,s.name_en AS service_name FROM bookings b LEFT JOIN services s ON s.id=b.service_id ${w.length?'WHERE '+w.join(' AND '):''} ORDER BY b.appointment_date DESC NULLS LAST,b.created_at DESC`; res.json((await query(sql,p)).rows); }catch(e){res.status(500).json({error:e.message});} });
...
// Super Admin only: real user/role management.
